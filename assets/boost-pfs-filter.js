...
const comingsoonTag = 'comingsoon';
const finalSaleTag = 'label:final-sale';
...
(function() {
	var onSale = false,
		...
		boostPFSRangeWidths = [180, 360, 540, 720, 900, 1080];
	...
	ProductGridItem.prototype.compileTemplate = function(data) {
		...
		var comingSoonClass = comingSoon ? boostPFSTemplate.comingsoon : '';

		itemHtml = itemHtml.replace(/{{soldOutClass}}/g, soldOutClass);
		itemHtml = itemHtml.replace(/{{saleClass}}/g, saleClass);
		itemHtml = itemHtml.replace(/{{comingSoonClass}}/g, comingSoonClass);
		...
		// Current Price
		itemHtml = itemHtml.replace(/{{itemCurrentPrice}}/g, buildCurrentPrice(data));
		// Compare Price
		itemHtml = itemHtml.replace(/{{itemComparePrice}}/g, buildComparePrice(data));

		// Add Custom Label
		itemHtml = itemHtml.replace(/{{itemCustomLabel}}/g, buildCustomLabel(data, true));
		itemHtml = itemHtml.replace(/{{itemFinalSale}}/g, buildFinalSaleLabel(data));
		...
		// itemHtml = itemHtml.replace(/{{notifyMe}}/g, notifyMe(data));
		itemHtml = itemHtml.replace(/{{itemDefaultButton}}/g, defaultButton(data));
		
		itemHtml = itemHtml.replace(/{{itemVariantId}}/g, firstAvaVariant && firstAvaVariant.id);
		itemHtml = itemHtml.replace(/{{itemVariantTitle}}/g, firstAvaVariant && firstAvaVariant.title);
		itemHtml = itemHtml.replace(/{{itemVariantsSize}}/g, onlyOneVariant(data));
		itemHtml = itemHtml.replace(/{{itemLabelsProperty}}/g, buildLabelsProperty(data));
		itemHtml = itemHtml.replace(/{{itemButton}}/g, buildButton(data));
		itemHtml = itemHtml.replace(/{{itemVariantsList}}/g, variantsList(data));
		itemHtml = itemHtml.replace(/{{itemVariantsOptions}}/g, variantOptions(data));
		itemHtml = itemHtml.replace(/{{notifyMeVariantsOptions}}/g, notifyMeVariantOptions(data));
		itemHtml = itemHtml.replace(/{{itemYotpoStars}}/g, getYotpoStars(data));
	...
	function buildButton(data){
		if(data.tags.includes(comingsoonTag)){
			return ``
		}else{
			return `<button class="primary-button" style="display: none;" type="submit">${boostPFSThemeConfig.label.quickadd}</button>`
		}
	}

	function buildLabelsProperty(data) {
		if(data.tags.includes(finalSaleTag)){
			return `<input type="hidden" name="properties[Labels]" value="Final Sale" />`
		}
		return '';
	}

	function onlyOneVariant(data){
		let classname = '';
	...
	// 	return `<div class="product-notifyme visually-hidden">
	// 	<div class='klaviyo-product-container' id='klaviyo-data-handle-${data.handle}' data-klaviyo-handle='${data.handle}'>
	...
	function defaultButton(data){
		if(!data || data.variants.length > 1 || !firstAvaVariant) return ''
		return `<span class="default-button add-to-cart-variant" data-value="${firstAvaVariant.id}">${boostPFSThemeConfig.label.quickadd} +</span>`
	}

	function variantsList(data){
		let list = '';
		if(data.tags.includes(comingsoonTag)) return list;
		if(data && data.variants.length && data.variants.length > 1){
			for(let variant of data.variants){
				list += `<li class="hover-select__item add-to-cart-variant ${variant.available ? '' : 'variant-disabled-notifyme'}" data-pid="`+data.id+`" data-ptitle="`+data.title+`" data-ptype="`+ data.product_type +`" data-pprice="`+ data.price_min +`" data-pbrand="`+data.vendor+`" data-value="${variant.id}">${variant.title}</li>`
			}
		}
		return list
	}

	function variantOptions(data){
		let options = '';
		if(data.tags.includes(comingsoonTag)) return '';
		if(data && data.variants.length){
			for(let variant of data.variants){
				options += `<option value="${variant.id}" class="select-tag-item" ${variant.available ? '' : 'disabled'} ${firstAvaVariant && firstAvaVariant.id === variant.id ? 'selected' : ''} >${variant.title}</option>`;
			}
		}
		return options
	}

	function notifyMeVariantOptions(data){
		let options = '';
		if(data.tags.includes(comingsoonTag)) return '';
		if(data && data.variants.length){
			for(let variant of data.variants){
				options += `<option value="${variant.id}"  ${variant.available ? 'disabled' : ''} ${firstAvaVariant && firstAvaVariant.id === variant.id ? 'selected' : ''}>${variant.title}</option>`;
			}
		}
		return options
	}

	function getYotpoStars(data){
		return `<div class="yotpo bottomLine" data-product-id="${data.id}" data-url="{{itemUrl}}"></div>`
	}
	...
	function buildCurrentPrice(data) {
		return data.pric
	}

	function buildComparePrice(data) {
	...
	// Custom Label
	function buildCustomLabel(data, showall) {
		if(!showall){
			let label = '';
			const firstAvailableVariant = firstVariant
			if(data.tags.includes(comingsoonTag)){
				label = boostPFSThemeConfig.label_basic.comingsoon;
			}else if(firstAvailableVariant.inventory_quantity > 0 && firstAvailableVariant.inventory_quantity < 5){
				label = boostPFSThemeConfig.label_basic.almost_gone;
			}else if(firstAvailableVariant.inventory_quantity === 0){
				label = boostPFSThemeConfig.label_basic.get_notified;
			}else if(data.tags.includes('pfs:label-back-in-stock')){
				label = boostPFSThemeConfig.label_basic.back_in_stock;
			}else if(data.tags.includes('pfs:label-new')){
				label = boostPFSThemeConfig.label_basic.new;
			}
			// Build Labels
			...
			const firstAvailableVariant = firstVariant
			if(data.tags.includes(comingsoonTag)){
				labels += `<span>${boostPFSThemeConfig.label_basic.comingsoon}</span>`;
			}
			if(firstAvailableVariant.inventory_quantity > 0 && firstAvailableVariant.inventory_quantity < 5){
				labels += `<span>${boostPFSThemeConfig.label_basic.almost_gone}</span>`;
			}
			if(firstAvailableVariant.inventory_quantity === 0){
				labels += `<span>${boostPFSThemeConfig.label_basic.get_notified}</span>`;
			}
			if(data.tags.includes('pfs:label-back-in-stock')){
				labels += `<span>${boostPFSThemeConfig.label_basic.back_in_stock}</span>`;
			}
			if(data.tags.includes('pfs:label-new')){
				labels += `<span>${boostPFSThemeConfig.label_basic.new}</span>`;
			}
			if(data.tags.includes('recycled')){
				labels += `<span>${boostPFSThemeConfig.label_basic.recycled}</span>`;
			}
			if(data.tags.includes('updated-fit')){
				labels += `<span>${boostPFSThemeConfig.label_basic.updated_fit}</span>`;
			}
			// Build Labels
			return labels;
		}
	...
	function buildFinalSaleLabel(data){
		var label = '';
		if(data.tags.includes('label:final-sale')){
			label = `<span class="sale-label">Final sale</span>`
		}
		return label
	}
	...
	ProductList.prototype.afterRender = function(data) {
		...
		if(typeof Yotpo !== 'undefined'){
			// REFRESHING YOTPO STAR RATING WIDGET
